...
int main(int argc, char* argv[])
{
    ...
    mcm_conn_param request = {};
    {
        ...
        std::snprintf(request.local_addr.port, sizeof(request.local_addr.port), "%d", 8002);
            ...
            //     return 1; // Handle the error as needed
            // }
            request.local_addr = {.ip = "192.168.1.21", .port = "8002"};
        ...
        request.payload_args.rdma_args.transfer_size =
            3840 * 2160 * 4; // Example transfer size
...
// Transmit bulk data in various payload sizes
...
for (size_t queue_size : queue_sizes) {
    for (size_t payload_size : payload_sizes) {
        // Setup RDMA TX Connection
        connection::RdmaTx *conn_tx = new connection::RdmaTx();
        EmulatedTransmitter *emulated_tx = new EmulatedTransmitter(ctx);

        // Update request with payload size and queue size
        request.payload_args.rdma_args.transfer_size = payload_size;
        request.payload_args.rdma_args.queue_size = queue_size;

        log::info("Configuring RDMA TX connection")("payload_size", payload_size)("queue_size", queue_size);
        auto res = conn_tx->configure(ctx, request, mDevHandle);
        if (res != connection::Result::success) {
        ...
        if (res != connection::Result::success) {
            log::error("Failed to establish RDMA TX connection")("result",
        ...
        if (res != connection::Result::success) {
            log::error("Configure EmulatedTransmitter failed")("result",
                                                               connection::result2str(res));
            delete conn_tx;
            delete emulated_tx;
            continue;
        ...
        if (res != connection::Result::success) {
            log::error("Establish EmulatedTransmitter failed")("result",
                                                               connection::result2str(res));
            delete conn_tx;
            delete emulated_tx;
            return 1;
        ...
        for (size_t i = 0; i < num_iterations; ++i) {
            ...
            emulated_tx->transmit_plaintext(ctx, test_data.data(), payload_size);
